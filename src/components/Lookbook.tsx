...
export function Lookbook({ onQuickView, shoes }: LookbookProps) {
  ...
  return (
    <section id="lookbook" className="py-14 sm:py-20 md:py-28 bg-white border-t border-zinc-200 overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        
        {/* Section Header */}
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-5 sm:gap-6 mb-6 sm:mb-8">
          <div className="space-y-2.5 sm:space-y-3 max-w-2xl">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-zinc-950 text-white text-[10px] sm:text-xs font-mono font-bold uppercase tracking-widest shadow-xs">
              <Camera className="size-3 sm:size-3.5 text-[#ccff00]" />
              <span>STREET CULTURE ARCHIVE</span>
            </div>
            <h2 
              className="text-3xl sm:text-5xl md:text-6xl font-black tracking-tight text-black uppercase leading-[1.05]"
              style={{ fontFamily: 'Syne, sans-serif' }}
            >
              DENCLUB ON THE STREETS
            </h2>
            <p className="text-xs sm:text-base text-zinc-600 max-w-xl leading-relaxed">
          ...
          <div className="flex flex-wrap items-center gap-3 shrink-0 pb-1">
            ...
            <div className="flex items-center p-1 bg-zinc-100 rounded-full border border-zinc-200 shadow-xs">
              <button
                ...
                className={`px-3 sm:px-4 py-1.5 sm:py-2 rounded-full text-xs font-bold uppercase flex items-center gap-1.5 transition cursor-pointer ${
        ...
        {/* ======================================================================= */}
        {viewMode === 'circular' && (
          <div className="relative w-full h-[360px] sm:h-[440px] md:h-[480px] rounded-3xl overflow-hidden bg-gradient-to-b from-zinc-50 via-zinc-100/70 to-white border border-zinc-200 shadow-xl flex items-center justify-center">
            
            {/* Guide Badge */}
            <div className="absolute top-2.5 sm:top-4 z-20 px-3 sm:px-4 py-1 sm:py-1.5 rounded-full bg-white/90 backdrop-blur-md border border-zinc-200 shadow-md text-[10px] sm:text-xs font-mono font-bold text-zinc-800 flex items-center gap-1.5 sm:gap-2">
              <RotateCw className="size-2.5 sm:size-3 text-[#84cc16] animate-spin" style={{ animationDuration: '6s' }} />
              <span className="truncate max-w-[240px] sm:max-w-none">DRAG OR SCROLL TO ROTATE 3D LOOKBOOK</span>
            </div>
